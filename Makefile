...
test: ## 🧪 Run tests with coverage
	GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_SYSTEM=/dev/null go tool gotestsum --format=short-verbose -- -race -covermode=atomic -coverprofile=coverage.txt ./...

test-quick: ## ⚡ Run tests without coverage
	GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_SYSTEM=/dev/null go tool gotestsum --format=short -- ./...
