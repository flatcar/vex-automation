...
**Status:** Phase 1 (PoC) implemented: the `generate` and `sync-glsa` CLI subcommands exist and produce a real OpenVEX document from a Flatcar SBOM and a local/synced GLSA mirror (see the repository's `internal/` packages and `README.md` for usage). Future Phases (rolling-forward, human overrides, CI wiring, etc.) are discussion-only — not committed, not sequenced, not necessarily "Phase 2" as a single next step.
